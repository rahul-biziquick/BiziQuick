...
const authenticate = async (req, res, next) => {
  ...
  try {
    ...
    if (!["WEB", "MOBILE"].includes(decoded.platform)) {
      logger.warn("Invalid platform in token", {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: {
        ...
        webRefreshTokenExpiresAt: true,
        mobileRefreshTokenExpiresAt: true,
        webSessionVersion: true,
        mobileSessionVersion: true,
      },
    });
    ...
    const storedRefreshToken =
      decoded.platform === "WEB"
        ? user.webRefreshToken
        : user.mobileRefreshToken;
    ...
    const currentSessionVersion =
      decoded.platform === "WEB"
        ...
        : user.mobileSessionVersion;

    if (!storedRefreshToken) {
      logger.warn("No active session for this platform", {
        correlationId: req.correlationId,
      });
    ...
    if (decoded.sessionVersion !== currentSessionVersion) {
      logger.warn("Token invalidated due to new session", {
        correlationId: req.correlationId,
      });
